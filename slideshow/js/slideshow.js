...
var createSlideshow = function () {
    ...
    return {
        ...
        setSlideSpeed : function () {
            let slideSpeed = parseFloat(window.prompt("Enter the new slide show speed in milliseconds", '1000'));
            if (isNan(slideSpeed)) {
                alert("enter speed in multiples of 500");
                this.setSlideSpeed();
            } else {
                let speed = parseFloat(slideSpeed); 
                stopSlideShow();
                time = setInterval(displayNextImage, speed);
            }
...
window.addEventListener("load", function () {
    ...
    // CHANGE THE SPEED
    $("speed").onclick = slideshow.setSlideSpeed(); 
});